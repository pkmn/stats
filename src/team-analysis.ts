...
export function teamStalliness(team: PokemonSet[]) {
  let teamBias = 0;
  const teamStalliness = [];
  for (const pokemon of team) {
    const {bias, stalliness} = pokemonStalliness(pokemon);
    teamBias += bias;
...
function pokemonStalliness(pokemon: PokemonSet) {
  ...
  let {bias, stalliness} = formeStalliness(pokemon);
  ...
  if (pokemon.species === 'meloetta' && pokemon.moves.includes('relicsong')) {
    ...
    stalliness = (stalliness + formeStalliness(pokemon).stalliness) / 2;
  } else if (
...
function formeStalliness(pokemon: PokemonSet) {
